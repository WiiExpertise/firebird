...
export function TweetsSidebar() {
  const tweets = [
    { username: "John Doe", handle: "johndoe", timestamp: "2h ago", content: "Just saw a huge fire downtown. Stay safe, everyone!" },
    { username: "Jane Smith", handle: "janesmith", timestamp: "3h ago", content: "Heavy floods in the coastal area. Avoid travel!" }
  ];

  return (
    <aside className="w-1/ bg-red-600/90 text-black p-4 rounded-lg shadow-md  right-6 top-32 h-[calc(100vh-8rem)] overflow-y-auto z-40"> 
      <h3 className="text-xl font-semibold mb-4 text-white">Latest Tweets</h3>
      <div className="space-y-4 flex flex-col">
        {tweets.map((tweet, index) => (
          <TweetCard key={index} username={tweet.username} handle={tweet.handle} timestamp={tweet.timestamp} content={tweet.content} />
        ))}
      </div>
    </aside>
  );
